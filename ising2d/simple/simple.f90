...
contains
    ...
    subroutine promedios(T, npromedio, S, mmedia, emedia, chi, c)
        ...
        m_curr = mean_spin(S)                 ! equivalente a np.mean(S)
    ...
    subroutine metropolis_pre(S, prob, dm, de)
        ! Una propuesta de flip en sitio aleatorio + Metropolis
        integer, intent(inout) :: S(:,:)
        real(8), intent(in)    :: prob(2)        ! [exp(-4β), exp(-8β)]
        ...
        a = 1 + int(min(0.999999999999D0, uni()) * dble(size(S,1)))
        b = 1 + int(min(0.999999999999D0, uni()) * dble(size(S,2)))
    ...
    subroutine init_spins_all_plus(S)
        integer, intent(out) :: S(:,:)
        S = 1
    end subroutine init_spins_all_plus
